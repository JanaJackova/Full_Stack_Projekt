...
const PersonForm = () => {
    ...
    const handleSubmit = (e) => {
        ...
        (id ? apiPut("/api/persons/" + id, person) : apiPost("/api/persons", person))
            ...
            .catch((error) => {
                ...
                if (error.response && error.response.status === 400 && error.response.data.message === "The identification number is already in the system") {
                    setError("IČO již existuje. Zadejte prosím jiné IČO.");
                } else {
    ...
    return (
        <div>
            <h1 className="text-headline">{id ? "Upravit" : "Vytvořit"} firmu</h1>
            <hr />
            {errorState ? (
